  ...
  try {
    ...
    if (debtSnapshot.exists()) {
      await firestoreUpdateDoc(debtDocRef, {
        ...
        verified: debtSnapshot.data().isPaid ? false : true,
      });
...
const verifyDebit = async (groupId, debtorId, verified) => {
  try {
    const debtDocRef = debtDocument(groupId, debtorId);
    const debtSnapshot = await firestoreGetDoc(debtDocRef);
    ...
    if (debtSnapshot.exists()) {
      await firestoreUpdateDoc(debtDocRef, {
        isPaid: verified,
        datePaid: verified ? new Date() : null,
        verified: verified,
      });
...
const setDebtAsVerified = async (groupId, debtorId) => {
  try {
    const debtDocRef = debtDocument(groupId, debtorId);
    const debtSnapshot = await firestoreGetDoc(debtDocRef);
    ...
    if (debtSnapshot.exists()) {
      ...
      console.log("Debt marked as verified successfully");
    } else {
      console.log("Debt not found");
    }
  } catch (error) {
...
const calculateDebtDistribution = (
  adminId: string,
  members: string[],
  sharedDebtAmount: number,
  divisionMethod: string
  ...
  if (divisionMethod === "equal") {
    ...
    const perMemberDebt =
      numberOfMembers > 0
        ? sharedDebtAmount / numberOfMembers
        : sharedDebtAmount;
...
const getGroupId = async (code) => {
  const snapshot = await getDocs(groupsCollection);
  const matchingGroup = snapshot.docs.find(
    (doc) => doc.data().groupIdInvite == code
  );
...
const deleteGroup = async (groupId: string) => {
  try {
    ...
    if (groupSnapshot.exists()) {
      ...
      console.log(
        `Removed the group (${groupId}) from the users' 'groups' arrays.`
      );
    } else {
      console.log("Group not found");
    }
    ...
    if (groupSnapshot.exists()) {
      ...
      if (groupData && Array.isArray(groupData.members)) {
        ...
        if (groupData.members.includes(userId)) {
          ...
          const newDebtAmountPerMember =
            sharedDebtAmount / remainingMembers.length;
          ...
          console.log(
            "User deleted from group successfully, and debts recalculated."
          );
        } else {
  ...
  try {
    ...
    if (groupSnapshot.exists()) {
      ...
      if (groupData && Array.isArray(groupData.members)) {
        ...
        const sharedDebtAmount = groupData.debtAmount;
        const newDebtAmountPerMember =
          sharedDebtAmount / groupData.members.length;
        const description = "Shared Debt Distribution";
        ...
        const updatedDebtAmountPerMember =
          sharedDebtAmount / groupData.members.length;
        const adminDebtData = {
          groupId,
...
const getPaidDebtsForUser = async (userId) => {
  ...
  return allDebts.filter(
    (debt) => debt.debtorId === userId && debt.isPaid == true
  );
};
